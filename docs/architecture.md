...
- `collection_index`: a local index of latest activity per collection with an
  activity-time ordering index, derived from `contains` relations and block
  timestamps for scalable collection list ordering.
